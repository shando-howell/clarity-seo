...
export const updateJobWithSnapshotId = mutation({
    ...
    handler: async (ctx, args) => {
        await ctx.db.patch(args.jobId, {
            ...
            status: "running",
            error: undefined, // Cear any previous error when job starts running
        });
        return null;
    },
});
...
export const retryJob = mutation({
    ...
    returns: v.null(),
    handler: async (ctx, args) => {
        // Reset the job to pending status and clear error, results, and old snaphotId
        await ctx.db.patch(args.jobId, {
            status: "pending",
            error: undefined,
...
/**
 * Check if a job can use smart rety (has analysis prompt and scraping data)
 */
